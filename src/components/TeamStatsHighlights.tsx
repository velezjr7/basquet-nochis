...
import { Shield, ShieldAlert, Target, TrendingUp, LucideIcon } from 'lucide-react';
...
const StatCard: React.FC<StatCardProps> = ({ title, teamName, value, icon: Icon, colorClass, label }) => (
  <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-start gap-4 hover:shadow-md transition-shadow">
    ...
    <div>
      ...
      <h4 className="text-lg font-bold text-gray-900 leading-tight mb-1">{teamName}</h4>
      <p className="text-sm font-medium text-gray-500">
        <span className="font-mono font-bold text-gray-900">{value}</span> {label}
      </p>
    </div>
  </div>
);
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <StatCard
        ...
        value={Math.round(bestOffense.pointsFor / (bestOffense.played || 1) * 10) / 10}
        label="pts/partido"
        icon={Target}
      ...
      <StatCard
        ...
        value={Math.round(bestDefense.pointsAgainst / (bestDefense.played || 1) * 10) / 10}
      ...
      <StatCard
        ...
        value={Math.round(worstDefense.pointsAgainst / (worstDefense.played || 1) * 10) / 10}
        label="recibidos/partido"
        icon={ShieldAlert}
